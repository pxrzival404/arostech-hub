  ...
  return new Promise((resolve, reject) => {
    ...
    if (data.description) {
      doc
        ...
        .fontSize(10)
        .font("Helvetica")
        .text(data.description, 50, currentY, { width: doc.page.width - 100, lineGap: 3 });
      currentY = doc.y + 16;
    }
    ...
    if (data.specifications && data.specifications.length > 0) {
      ...
      doc.rect(50, currentY - 5, tableWidth, 25).fill("#f0fdf4");
    ...
    if (data.highlights && data.highlights.length > 0) {
      ...
      for (const rawHl of data.highlights) {
        ...
        doc.circle(58, currentY + 7, 3).fill("#059669");
        doc.fontSize(10).font("Helvetica").fillColor("#374151")
          .text(hl, 70, currentY, { width: doc.page.width - 170 });
    ...
    currentY += 30;
    doc.roundedRect(50, currentY, doc.page.width - 100, 40, 4).fill("#f3f4f6");
    doc.fontSize(8).font("Helvetica").fillColor("#6b7280")
      .text("Spesifikasi teknis bersifat indikatif dan dapat berubah tanpa pemberitahuan sebelumnya. Hubungi tim sales kami untuk informasi terbaru.", 62, currentY + 8, { width: doc.page.width - 124, lineGap: 2 });
    doc.fontSize(8).font("Helvetica-Bold").fillColor("#059669")
      .text("info@dayaberkah.id | +62 822-3026-1340", 62, currentY + 26, { width: doc.page.width - 124 });
...
/**
 * GET /api/products/[slug]/spec-pdf
 * Alias endpoint — same logic as /api/spec-pdf/[slug]
 */
export async function GET(
  ...
  try {
    ...
    // ─── Fallback: primary method's data missing — try alternatives ───
    if (hasSpecs) {
      const pdfBuffer = await generateSpecPdf({
        productName: product.name,
        subcategory: product.subcategory,
    ...
    console.error("[Product Spec PDF API] Error:", error);
    return NextResponse.json(
