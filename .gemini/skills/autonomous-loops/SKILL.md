...
# Autonomous Loops activate_skill
...
claude -p "read_file the spec in docs/auth-spec.md. Implement OAuth2 login in src/auth/. write_file tests first (TDD). Do NOT create any new documentation files."
...
claude -p "read_file .claude-context.md for priorities. Work through them in order."
rm .claude-context.md
```

**With `--allowedTools` restrictions:**
```bash
# read_file-only analysis pass
claude -p --allowedTools "read_file,grep_search,glob" "Audit this codebase for security vulnerabilities..."

# write_file-only implementation pass
claude -p --allowedTools "read_file,write_file,replace,run_shell_command" "Implement the fixes from security-audit.md..."
```
...
│ Scan output dir      │  deploys   │ read_file assigned number  │
...
PHASE 1: read_file and deeply understand the specification.
...
| Stage | Model | Purpose |
|-------|-------|---------|
| Research | Sonnet | read_file codebase + RFC, produce context doc |
| Plan | Opus | Design implementation steps |
| Implement | Codex | write_file code following the plan |
