...
│       ▼              ▼             ▼                ▼        │
│  ┌──────────────────────────────────────────────────────┐    │
│  │              ECC activate_skill + Agent Layer                  │    │
│  │                                                      │    │
...
Use write_todos for in-session task tracking

# Medium-term: project memory files
write_file to ~/.claude/projects/*/memory/ for cross-session recall
...
write_file task queue to ~/.claude/projects/*/memory/task-queue.md
...
| Orchestration | ECC skills + agents | activate_skill definitions direct agent behavior |
...
| Task Queue | Memory-persisted task list | write_todos + memory files |
...
claude -p "Create a scheduled task: every weekday at 9am, review my GitHub notifications, open PRs, and calendar. write_file a morning briefing to memory."
...
1. read_file calendar event details
...
5. write_file prep doc to memory
```
