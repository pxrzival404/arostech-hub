---
name: gateguard
description: Fact-forcing gate that blocks replace/write_file/run_shell_command (including MultiEdit) and demands concrete investigation (importers, data schemas, user instruction) before allowing the action. Measurably improves output quality by +2.25 points vs ungated agents.
origin: community
---
...
But asking "list every file that imports this module" forces the LLM to run grep_search and read_file. The investigation itself creates context that changes the output.

**Three-stage gate:**

```
1. DENY  — block the first replace/write_file/run_shell_command attempt
...
### replace / MultiEdit Gate (first edit per file)
...
1. List ALL files that import/require this file (use grep_search)
...
### write_file Gate (first new file creation)
...
### Destructive run_shell_command Gate (every destructive command)

Triggers on: `rm -rf`, `git reset --hard`, `git push --force`, `drop table`, etc.

```
1. List all files/data this command will modify or delete
2. write_file a one-line rollback procedure
3. Quote the user's current instruction verbatim
```

### Routine run_shell_command Gate (once per session)
