...
2. **write_file a test** — Use `#[test]` in a `#[cfg(test)]` module, rstest for parameterized tests, or proptest for property-based tests
3. **Mock dependencies** — Use mockall to isolate the unit under test
4. **Run tests (RED)** — Verify the test fails with the expected error
5. **Implement (GREEN)** — write_file minimal code to pass
...
```
RED     → write_file a failing test first
GREEN   → write_file minimal code to pass the test
...
```rust
// RED: write_file test first, use todo!() as placeholder
...
**Remember**: Tests are documentation. They show how your code is meant to be used. write_file them clearly and keep them up to date.
