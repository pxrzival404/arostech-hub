...
function resolveSubHook(modulePath) {
  try {
...
const runBlockNoVerify = resolveSubHook('./block-no-verify');
const runAutoTmuxDev = resolveSubHook('./auto-tmux-dev');
const runTmuxReminder = resolveSubHook('./pre-bash-tmux-reminder');
const runGitPushReminder = resolveSubHook('./pre-bash-git-push-reminder');
const runCommitQuality = resolveSubHook('./pre-bash-commit-quality');
const runGateGuard = resolveSubHook('./gateguard-fact-force');
const runCommandLog = resolveSubHook('./post-bash-command-log');
const runPrCreated = resolveSubHook('./post-bash-pr-created');
const runBuildComplete = resolveSubHook('./post-bash-build-complete');
...
const PRE_BASH_HOOKS = [
  {
    ...
    run: rawInput => (runBlockNoVerify ? runBlockNoVerify(rawInput) : null),
  },
  {
    id: 'pre:bash:auto-tmux-dev',
    run: rawInput => (runAutoTmuxDev ? runAutoTmuxDev(rawInput) : null),
  },
  {
    id: 'pre:bash:tmux-reminder',
    profiles: 'strict',
    run: rawInput => (runTmuxReminder ? runTmuxReminder(rawInput) : null),
  },
  {
    id: 'pre:bash:git-push-reminder',
    profiles: 'strict',
    run: rawInput => (runGitPushReminder ? runGitPushReminder(rawInput) : null),
  },
  {
    id: 'pre:bash:commit-quality',
    profiles: 'strict',
    run: rawInput => (runCommitQuality ? runCommitQuality(rawInput) : null),
  },
  {
    id: 'pre:bash:gateguard-fact-force',
    profiles: 'standard,strict',
    run: rawInput => (runGateGuard ? runGateGuard(rawInput) : null),
  },
];
...
const POST_BASH_HOOKS = [
  {
    ...
    run: rawInput => (runCommandLog ? runCommandLog(rawInput, 'audit') : null),
  },
  {
    id: 'post:bash:command-log-cost',
    run: rawInput => (runCommandLog ? runCommandLog(rawInput, 'cost') : null),
  },
  {
    id: 'post:bash:pr-created',
    profiles: 'standard,strict',
    run: rawInput => (runPrCreated ? runPrCreated(rawInput) : null),
  },
  {
    id: 'post:bash:build-complete',
    profiles: 'standard,strict',
    run: rawInput => (runBuildComplete ? runBuildComplete(rawInput) : null),
  },
];
