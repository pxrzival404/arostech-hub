...
| **`graphify`** | `graphify` | `call_mcp_tool('graphify', 'query_graph', { question: 'src/lib/auth.ts', mode: 'bfs', depth: 3, token_budget: 2000 })` | Codebase structural ontology, God node detection, and dependency impact analysis. |
| **`shortest_path`** | `graphify` | `call_mcp_tool('graphify', 'shortest_path', { source, target, max_hops })` | Find the shortest dependency and execution path between two concepts in the knowledge graph. |
| **`god_nodes`** | `graphify` | `call_mcp_tool('graphify', 'god_nodes', {})` | Identify architectural hub nodes with the highest degree of incoming/outgoing connections (high blast radius). |
| **`get_node`** | `graphify` | `call_mcp_tool('graphify', 'get_node', { label })` | Retrieve full details, source location, and degree for a specific graph node by label or ID. |
| **`get_neighbors`** | `graphify` | `call_mcp_tool('graphify', 'get_neighbors', { label, relation_filter, token_budget })` | Retrieve all direct incoming and outgoing neighbor nodes with edge relations and confidence scores. |
| **`get_community`** | `graphify` | `call_mcp_tool('graphify', 'get_community', { community_id, token_budget })` | Retrieve all nodes within an architectural cluster/community by community ID. |
| **`graph_stats`** | `graphify` | `call_mcp_tool('graphify', 'graph_stats', {})` | Retrieve knowledge graph topological statistics (node count, edge count, density, modularity). |
...
| **`graphify path`** | `CLI / TUI` | `graphify path "<source>" "<target>"` | Graphify CLI Path Tracer: Find the shortest connection path between two named concepts from terminal. |
...
| **`graphify reflect`** | `Workflow` | `graphify reflect [--if-stale] [--out docs/LESSONS.md] [--graph graphify-out/graph.json]` | Graphify Work Memory: Aggregate agent Q&A lessons into LESSONS.md and create .graphify_learning.json overlay. |
| **`graphify extract --code-only`** | `Workflow` | `graphify extract <directory> --code-only [--directed] [--no-viz]` | Graphify AST Extraction: Deterministically parse local codebase AST without requiring LLM vision or API keys. |
