> [!CAUTION]
> **Archived Historical Proposal (2026-08-06):** This auth implementation plan was superseded by custom Edge subdomain routing and light auth guards in `src/middleware.ts` + `src/lib/auth/auth-guard.ts`. Retained for historical reference only.
