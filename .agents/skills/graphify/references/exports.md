...
> **Note:** Serving graphify over MCP requires the `mcp` extra package. If `mcp` is missing, install/upgrade via `uv tool install --upgrade "graphifyy[mcp]"`.

```bash
$(cat graphify-out/.graphify_python) -m graphify.serve graphify-out/graph.json
```

This starts a stdio MCP server that exposes tools: `query_graph`, `get_node`, `get_neighbors`, `get_community`, `god_nodes`, `graph_stats`, `shortest_path`, `list_prs`, `get_pr_impact`, `triage_prs`.

For workspace-scoped agent configuration in Antigravity, save the configuration in `.agents/mcp.json` or `.agents/mcp_config.json`:
