...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value ECC-native skills, hooks, agents, and operator workflows. Use when the user wants help understanding what capabilities are actually available in their environment.
metadata:
  origin: ECC
---
...
- User says "audit workspace surface", "recommend automations", "what plugins or MCPs should I use?", or "what am I missing?"
- Auditing a machine or repo before installing more skills, hooks, or connectors
- Comparing available marketplace plugins against workspace capabilities
- Reviewing `.env`, `mcp_config.json`, plugin settings, or connected-app surfaces to find missing workflow layers
...
1. Repo surface
   ...
   - `.agents/settings.json`, `~/.gemini/config/config.json`, `~/.gemini/config/mcp_config.json`
   - `AGENTS.md`, `GEMINI.md`, `.agents/hooks.json`, install manifests
